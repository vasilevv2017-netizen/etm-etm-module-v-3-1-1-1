...
const App: React.FC = () => {
  ...
  // Use any for Bluetooth refs to fix missing Web Bluetooth type definitions
  ...
  const parseLawicellFrame = (frame: string) => {
    ...
    try {
      ...
      const dlc = parseInt(frame.substring(1 + idLen, 2 + idLen), 16);
  ...
  const handleCharacteristicValueChange = (event: Event) => {
    // Cast to any to bypass missing BluetoothRemoteGATTCharacteristic type
  ...
  // Use any for device parameter to bypass missing BluetoothDevice type
