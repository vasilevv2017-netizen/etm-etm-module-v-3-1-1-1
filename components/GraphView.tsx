...
const Chart: React.FC<ChartProps> = ({ config, messages, onConfigOpen }) => {
  ...
  // Sync zoom offset when config changes (if not in auto-scale)
  useEffect(() => {
    if (!autoScale) {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    // Only trigger long press if not clicking UI buttons
      ...
      {editingConfig && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 bg-black/95 backdrop-blur-2xl">
          <div className="w-full max-w-sm bg-gray-900 border border-white/10 rounded-2xl p-7 shadow-2xl">
            <h3 className="text-xl font-black text-white uppercase mb-5 tracking-tight">Graph Settings</h3>
            <div className="space-y-4">
              <div>
                <label className="text-[9px] font-mono text-gray-600 uppercase mb-1 block">Label</label>
                <input 
                  type="text" 
                  value={editingConfig.label} 
                  onChange={e => setEditingConfig({...editingConfig, label: e.target.value})} 
                  className="w-full bg-black/60 border border-white/5 rounded-lg p-3 font-mono text-gray-100" 
                />
              </div>
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-black/60 border border-white/5 rounded-lg p-3 font-mono text-red-500" 
                />
              </div>
              ...
              <div className="flex gap-2.5 pt-3">
                <button 
                  ...
                  className="flex-1 py-4 bg-red-600 text-white font-black rounded-xl uppercase text-[10px] shadow-lg shadow-red-900/30"
                >
                  Save
                </button>
                <button 
                  onClick={() => setEditingConfig(null)}
                  className="flex-1 py-4 bg-gray-800 text-gray-400 font-black rounded-xl uppercase text-[10px] border border-white/5"
                >
                  Cancel
                </button>
              </div>
