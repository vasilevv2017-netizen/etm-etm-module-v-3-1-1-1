...
const BluetoothConnection: React.FC<Props> = ({ onConnect }) => {
  ...
  const startScan = async () => {
    ...
    setError(null);
    try {
      // Список UUID, которые часто используются Bluetooth-Serial адаптерами
  ...
  return (
    <div className="flex-1 flex flex-col p-6 bg-gray-900 overflow-hidden">
      ...
      <div className="mt-12 flex-1 bg-gray-800/40 backdrop-blur-sm rounded-2xl border border-white/5 overflow-hidden mb-6 shadow-2xl flex flex-col items-center justify-center p-8 text-center">
        {isScanning ? (
          ...
          <div className="space-y-6">
            ...
            <p className="text-gray-500 text-[10px] px-4 leading-relaxed uppercase">Для работы на Android необходимо разрешить доступ к Bluetooth и Местоположению в настройках телефона.</p>
      ...
      <button
        onClick={startScan}
        disabled={isScanning}
        className="w-full py-5 bg-red-600 hover:bg-red-500 text-white font-black rounded-xl transition-all shadow-xl shadow-red-900/20 disabled:opacity-50 active:scale-[0.98] uppercase tracking-widest text-sm border-t border-white/20"
      >
